...
void testCreateNode() {
    ...
    if (node != nullptr && 
        node->word == word && 
        node->documentIds.size() == 1 && 
        node->documentIds[0] == docId &&
        node->parent == nullptr &&
        node->left == nullptr &&
        node->right == nullptr) {
...
void testInsertAndSearch() {
    ...
    // Confere para "banana"
    if (result1.found != 1 || result1.documentIds.size() != 2 || result1.documentIds[0] != 1 || result1.documentIds[1] != 4) {
        std::cout << "FALHA: Problema ao buscar 'banana'" << std::endl;
        success = false;
    }
    // Confere para "morango"
    if (result2.found != 1 || result2.documentIds.size() != 1 || result2.documentIds[0] != 2) {
        std::cout << "FALHA: Problema ao buscar 'morango'" << std::endl;
        success = false;
    }
    // Confere para "abacaxi"
    if (result3.found != 1 || result3.documentIds.size() != 1 || result3.documentIds[0] != 3) {
    ...
    if (success) {
        std::cout << "SUCESSO: Todas as buscas retornaram resultados esperados" << std::endl;
    }
...
void testInsertNode() {
    ...
    if (tree->root->documentIds.size() != 2) {
        std::cout << "FALHA: DocumentIds duplicados nao foram adicionados" << std::endl;
        success = false;
    }
...
void testDestroyTree() {
    ...
    SearchResult result = BST::search(tree, "banana");
    if (result.found == 0 && result.documentIds.empty()) {
        std::cout << "SUCESSO: Árvore destruída corretamente (acesso inválido detectado)" << std::endl;
    } else {
        std::cout << "FALHA: Acesso à árvore destruída retornou dados" << std::endl;
    }
